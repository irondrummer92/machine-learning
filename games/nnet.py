...
weightLookup = {"random":lambda x:0.01,
               "xavier":lambda x:1./np.sqrt(np.float(x)),
               "he.et.al":lambda x:np.sqrt(2./np.float(x))}
    ...
    def backProp(self, numEpochs, batchSize, learnRate,
        ...
        for i in range(numEpochs):
            ...
            if((i + 1) % stepError == 0):
                ...
                mseTrain = self.computeError(trainInput, trainOutput)
                mseTest = self.computeError(testInput, testOutput)
                
                # self.cvError[((i + 1) / stepError)-1] = [mseTrain,mseTest]
                self.cvError = np.append(self.cvError,mseTrain)
